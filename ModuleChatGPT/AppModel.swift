...
//  ModuleChatGPT
//
//  Created by Prathmesh Parteki on 07/12/24.
//
...
final class AppModel : ObservableObject {
    ...
    @Published var selectedModules : Modules?
    ...
    var isEmptyNewConceptScreen : Bool { !isThinking && generatedConcept.isEmpty }
    var hasResultNewConceptScreen : Bool { !isThinking && !generatedConcept.isEmpty }
    ...
    var isEmptyDefintionScreen : Bool { !isThinking && generatedDefinition.isEmpty }
    ...
    func send(text:String ,completion :@escaping(String)->Void) {
        ...
        DispatchQueue.main.async{
            self.client?.sendCompletion(with: text,maxTokens: 100 ,completionHandler: { result in
                switch result {
    ...
    func makeNewChat() {
        send(text: "\(newChatEntryText)") { output in
            ...
            print(self.generatedNewChat)
            self.isThinking = false
        }
    }
    func makeConcept() {
        send(text: "Generate a concept, generally a word or many, in the realm of anything, that is grounded in reality, and that may or may not valuable to learn about. Simply provide the short concept without punctuation") { output in
            DispatchQueue.main.async {
    ...
    func makeRelatedTopics() {
        send(text: "Generate 5 topics that are closely related to \(relatedTopicsEntryText).Simply provide the related topics seperated by new line.") { output in
            DispatchQueue.main.async {
    ...
    func makeArticle() {
        send(text: "Generate an in-depth, grounded in reality, 1-paragraph wikipedia article for a reader who does not understand the topic of \(articleEntryText)") { output in
            DispatchQueue.main.async {
    ...
    func makeNext() {
        send(text: "Provide a random but important, single thing to think about on the topic of daily living,in the form of direction for me to follow right now. Be specific.") { output in
            DispatchQueue.main.async {
    ...
    func makeAffirmation() {
        send(text: "Generate an affirmation I can tell myself that will subtly fill me with life and smiles and that is not to confusing. Provide the affirmation without quotes.") { output in
            DispatchQueue.main.async {
